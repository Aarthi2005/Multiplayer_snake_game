...
with open(r"C:\Users\aarth\OneDrive\Desktop\CN PACK\multiplayer-snake2\multiplayer-snake\client\preferences.json") as f:
    SETTINGS = json.load(f)

with open(r"C:\Users\aarth\OneDrive\Desktop\CN PACK\multiplayer-snake2\multiplayer-snake\client\screen_sizes.json") as f:
    SIZES = json.load(f)
...
class Cube:
    ...
    def draw(self, surface, board_size, y_offset) -> None:
        ...
        # Correctly map cube to the grid position
        x_pos = self.x * tile_size
        y_pos = self.y * tile_size + y_offset

        # Draw the cube within the grid's boundaries
        pygame.draw.rect(surface, self.color, (x_pos + 1, y_pos + 1, tile_size - 2, tile_size - 2))
...
class Snake:
    ...
    def check_apple_eaten(self, apple) -> bool:
        apple_x = apple.cube.x
        apple_y = apple.cube.y
...
class Apple:
    ...
    def regenerate_coords(self, snake, board_size: int) -> None:
        while True:
            ...
            # Ensure apple doesn't regenerate on top of the snake
            if not any(block.x == x and block.y == y for block in snake.coords):
                self.cube.x = x
                self.cube.y = y
                return
...
class Game:
    def __init__(self, surface):
        ...
        self.opponent_board_text = gui_text.Text("Enemy board:", FONT, (255, 0, 0), (WIDTH+70, OPPONENT_OFFSET- 30))
    ...
    """
    def draw_grid(self,x_offset=0, y_offset=0) -> None:
        ...
        for _ in range(self.board_size + 1):
            ...
            pygame.draw.line(self.surface, (100, 100, 100), (x_offset, y), (x+WIDTH, y))
    ...
    # Update the score text
        ...
        self.opponent_score_text.change_text(f"Enemy Score: {len(self.opponent_board) - 1} / {self.apple_goal}")

        if not isinstance(self.score_text.text, str):
            print(f"Score text is not a string: {self.score_text.text}")
            return  # Exit the method if it's not a string

    # Render the score texts as Pygame surfaces
        score_surface = FONT.render(self.score_text.text, True, (255, 255, 255))
        opponent_score_surface = FONT.render(self.opponent_score_text.text, True, (255, 0, 0))
    ...
    # Set positions for the rectangles
        score_rect.topleft = (WIDTH - 70, PLAYER_OFFSET // 2)
        opponent_score_rect.topleft = (WIDTH - 70, OPPONENT_OFFSET - 30)

    # Draw the text surfaces on the main surface
        self.surface.blit(score_surface, score_rect)
        self.surface.blit(opponent_score_surface, opponent_score_rect)
    ...
    def show_end_screen(self, message, color) -> None:
        ...
        # Get the rect for the rendered text surface, for centering
        text_rect = text_surface.get_rect()
        text_rect.center = (WIDTH // 2, HEIGHT - WIDTH - 50)

        # Blit the text surface onto the main surface at the desired position
...
def main():
    ...
    # Adjust the width and height to be a percentage of the screen size
    width = int(screen_width * 0.8)  # Set to 80% of screen width
    height = int(screen_height * 0.8)  # Set to 80% of screen height
